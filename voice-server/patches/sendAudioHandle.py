...
# 预缓冲包数量，直接发送。10≈600ms 緩衝：吸收 WiFi/CPU 抖動，減少播放卡頓/雜訊。
# 不增加首字延遲（首包仍立即送），只是給裝置更大的播放緩衝墊。
PRE_BUFFER_COUNT = 10
...
async def send_tts_message(conn: "ConnectionHandler", state, text=None):
    ...
    # Hermes 持續對話：回應(TTS)播完後，自動讓裝置重新聆聽（免再喚醒），
    # 沉默太久沒人講話才回待命。
    if state == "stop":
        await _hermes_continuous_listen(conn)


async def _hermes_continuous_listen(conn):
    """回應結束 → 送 listen start 讓裝置免喚醒繼續聽；沉默 N 秒沒新對話 → 送 listen stop 回待命。
    用 epoch 整數令牌 + 取消舊 watcher，避免 task 洩漏與多輪/插嘴時誤把裝置踢回待命。"""
    try:
        if not getattr(conn, "_hermes_continuous", True):
            return
        if getattr(conn, "websocket", None) is None:
            return
        timeout = float(getattr(conn, "_hermes_silence_timeout", 6))
        conn._hermes_in_turn = False  # 這一輪回應已送完
        conn._hermes_in_continuous = True  # 進入「免喚醒接話視窗」→ 啟用聲紋過濾(A)
        await conn.websocket.send(json.dumps({
            "type": "listen", "state": "start", "mode": "auto",
            "session_id": conn.session_id}))
        try:
            conn.logger.bind(tag="HERMES").info(
                f"🎙️ 持續對話：回應結束→送 listen start（{timeout}秒沒講話才待命）")
        except Exception:
            pass
        # 取消上一個還在睡的 watcher，避免堆積
        prev = getattr(conn, "_hermes_standby_task", None)
        if prev is not None and not prev.done():
            prev.cancel()
        conn._hermes_listen_epoch = getattr(conn, "_hermes_listen_epoch", 0) + 1
        epoch = conn._hermes_listen_epoch

        async def _standby_watch(ep):
            try:
                await asyncio.sleep(timeout + 0.3)
            except asyncio.CancelledError:
                return
            # 只有「仍是同一個 epoch」+「沒在講話」+「沒在處理新一輪」才回待命
            if (getattr(conn, "_hermes_listen_epoch", None) == ep
                    and not getattr(conn, "client_is_speaking", False)
                    and not getattr(conn, "_hermes_in_turn", False)
                    and getattr(conn, "websocket", None) is not None):
                conn._hermes_in_continuous = False  # 視窗超時回待命 → 關閉聲紋過濾(下次要喚醒詞)
                conn._hermes_locked_speaker = None   # 對話結束 → 清掉鎖定身份，下次喊 jarvis 重新判斷(Siri 式)
                try:
                    await conn.websocket.send(json.dumps({
                        "type": "listen", "state": "stop", "session_id": conn.session_id}))
                except Exception:
                    pass

        conn._hermes_standby_task = asyncio.create_task(_standby_watch(epoch))
    except Exception:
        pass
    ...
    try:
        ...
        if text.strip().startswith("{") and text.strip().endswith("}"):
            ...
            if isinstance(parsed_data, dict) and "content" in parsed_data:
                ...
                # 保存说话人信息到conn对象
                # 【Siri 式身份判斷】只在「喚醒後的第一句」(不在持續對話視窗內)用聲紋判斷身份、
                # 並鎖定整段對話；持續對話的後續句不再重判(聲紋每輪會飄、還會被電視/背景人聲觸發)
                # → 沿用喚醒當下鎖定的身份。視窗超時回待命時會清掉鎖定，下次喊 jarvis 重新判斷。
                if "speaker" in parsed_data:
                    if not getattr(conn, "_hermes_in_continuous", False):
                        conn.current_speaker = parsed_data["speaker"]
                        conn._hermes_locked_speaker = parsed_data["speaker"]
                    elif getattr(conn, "_hermes_locked_speaker", None) is not None:
                        conn.current_speaker = conn._hermes_locked_speaker
