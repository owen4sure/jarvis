        ...
        try:
            ...
            while len(conn.client_audio_buffer) >= 512 * 2:
                ...
                if client_have_voice:
                    # 偵測到「開始講話」的第一刻（轉折），就立刻通知 dashboard 顯示「正在聆聽」，
                    # 不必等 VAD 靜默 + ASR 辨識完才顯示 → 反應快很多。每句只送一次。
                    if not conn.client_have_voice:
                        try:
                            import threading as _th, urllib.request as _u, json as _j

                            def _notify():
                                try:
                                    _u.urlopen(_u.Request(
                                        "http://host.docker.internal:8811/api/event",
                                        data=_j.dumps({"type": "listening"}).encode(),
                            ...
                            pass
                        # 開口第一刻設「喚醒待抬頭」持久旗標 → 頭部 poller 一定抓得到(喊jarvis一開口就抬頭+找臉)。
                        try:
                            conn._hermes_wake_pending = time.time()
                        except Exception:
                            pass
