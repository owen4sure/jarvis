...
class VisionHandler(BaseHandler):
    ...
    async def handle_post(self, request):
        ...
        try:
            ...
            # Hermes: 存下裝置實際拍的圖（last_camera.jpg = live 來源、掛載出來給 dashboard）
            try:
                with open("/opt/xiaozhi-esp32-server/data/last_camera.jpg", "wb") as _f:
                    _f.write(image_data)
            except Exception:
                pass

            # Hermes Live：純擷取模式 → 只存影像、不跑 Gemini（讓 live 連拍夠快）
            if question.strip() == "__capture__":
                return web.Response(
                    text=json.dumps({"success": True, "action": Action.RESPONSE.name,
                                     "response": "ok"}, separators=(",", ":")),
                    content_type="application/json")
