...
# Hermes：direct_answer 會把「聊天回覆」變成 function call，而 Gemini 的 function call 參數是
# 「一次到齊」不串流 → 語音要等整段生成完才開口（無法邊生成邊講）。
# Gemini 2.5 flash 夠聰明，不需要 direct_answer 防誤觸工具 → 關掉它，讓聊天走【純文字串流】，
# TTS 就能邊生成邊逐句講，大幅降低開口延遲。
# 若關掉後語音「該叫工具卻不叫 / 亂叫工具 / 不回話」→ 把這個改回 True 即可（兩處都會跟著恢復）。
...
class ConnectionHandler:
    ...
    async def handle_connection(self, ws: websockets.ServerConnection):
        try:
            ...
            # 啟動「待播語音」輪詢 → 提醒到點時讓 StackChan 主動開口叫使用者
            self.voice_reminder_task = asyncio.create_task(self._voice_reminder_poller())
    ...
    async def _save_and_close(self, ws):
        """保存记忆并关闭连接"""
        try:
            # Hermes：原本「生成聊天標題」走 web 控制面板（manage_api 為 None）一定失敗狂噴錯誤，
            # 已停用 → 連執行緒都不再開，省資源、log 乾淨。
    ...
    async def _route_message(self, message):
        ...
        if isinstance(message, str):
            # 裝置喚醒(喊jarvis)會送 listen start → 設「待抬頭」旗標，頭部 poller 在喊jarvis的當下就抬頭。
            try:
                import json as _lj
                _lm = _lj.loads(message)
                if _lm.get("type") == "listen" and _lm.get("state") == "start":
                    self._hermes_wake_pending = time.time()
            except Exception:
                pass
    ...
    def _inject_tool_call_fewshot(self):
        ...
        if not DIRECT_ANSWER_ENABLED:
            return  # 關掉 direct_answer 時，這組 few-shot 教的就是它，一併略過避免教不存在的工具
        ...
        # 示例1：direct_answer（繁體，並釘住 Hermes 身份）
        da_tc_id = "fewshot_da_001"
        self.dialogue.put(Message(role="user", content="你是誰？", is_temporary=True))
        self.dialogue.put(Message(
            role="assistant",
            tool_calls=[{
                "id": da_tc_id,
                "function": {"arguments": '{"response": "我是 Hermes，你的夥伴～有什麼想聊的嗎？"}', "name": "direct_answer"},
        ...
        self.dialogue.put(Message(
            ...
            content="已直接回覆", is_temporary=True,
        ))

        # 示例1b：硬體動作（釘住「有身體、叫我動就呼叫工具」）
        if "self_robot_set_head_angles" in tool_names:
            hd_id = "fewshot_head_001"
            self.dialogue.put(Message(role="user", content="轉頭看右邊", is_temporary=True))
            self.dialogue.put(Message(
                role="assistant",
                tool_calls=[{
                    "id": hd_id,
                    "function": {"arguments": '{"yaw": 80, "pitch": 45}', "name": "self_robot_set_head_angles"},
                    "type": "function", "index": 0,
                }],
                is_temporary=True,
            ))
            self.dialogue.put(Message(
                role="tool", tool_call_id=hd_id,
                content='{"ok": true}', is_temporary=True,
            ))
            self.dialogue.put(Message(
                role="assistant", content="好，轉過去囉！", is_temporary=True,
            ))
        ...
        if "handle_exit_intent" in tool_names:
            ...
            self.dialogue.put(Message(
                ...
                tool_calls=[{
                    "id": tc_id,
                    "function": {"arguments": '{"say_goodbye": "掰掰，下次再聊~"}', "name": "handle_exit_intent"},
                    "type": "function", "index": 0,
                }],
                is_temporary=True,
            ))
            self.dialogue.put(Message(
                ...
                content="退出意圖已處理", is_temporary=True,
            ))
            self.dialogue.put(Message(
                role="assistant", content="掰掰，下次再聊~", is_temporary=True,
            ))
    ...
    def _look_at_user(self, query):
        """對話時轉頭面向使用者（yaw=0 正中）。若使用者本來就在叫他做動作則跳過。"""
        try:
            if not query or not (hasattr(self, "loop") and self.loop):
                return
            if any(k in query for k in (
                "轉頭", "轉向", "看左", "看右", "看向", "抬頭", "低頭",
                "跳舞", "搖頭", "左邊", "右邊", "上面", "下面", "頭轉")):
                return
            ...
            async def _do():
                # ① 先變「思考」臉（秒回饋）：你一講完它就進入思考臉 → 韌體會顯示能量漩渦動畫
                #    (思考專屬動畫)，讓你看到它在認真處理。回應好了情緒臉會關掉漩渦。
                for an in ("self.display.set_avatar", "self_display_set_avatar"):
                    try:
                        if mcp.has_tool(an):
                            await call_mcp_tool(
                                self, mcp, an,
                                _j.dumps({"face": "thinking"}), timeout=4)
                            break
                    except Exception:
                        pass
                # ② LED 變「思考中」呼吸藍 → 即使臉不夠明顯，燈圈也清楚告訴你它在處理
                for ln in ("self.led.set_all", "self_led_set_all"):
                    try:
                        if mcp.has_tool(ln):
                            await call_mcp_tool(
                                self, mcp, ln,
                                _j.dumps({"r": 0, "g": 40, "b": 120}), timeout=4)
                            break
                    except Exception:
                        pass
                # ③ 轉頭面向使用者（yaw=0 正面、pitch=45 水平，實測這高度剛好看到坐在前面的 Owen）
                import os as _os2
                _lp = int(_os2.environ.get("HERMES_LOOK_PITCH", "45"))
                for n in ("self.robot.set_head_angles", "self_robot_set_head_angles"):
                    try:
                        if mcp.has_tool(n):
                            await call_mcp_tool(
                                self, mcp, n,
                                _j.dumps({"yaw": 0, "pitch": _lp}), timeout=6)
                            break
                    except Exception:
                        pass
    ...
    def _hermes_log_chat(self, reply):
        """把這輪(原始問句 + 最終回覆)記進 dashboard。每輪只記一次；所有回覆路徑(純文字/工具REQLLM/
        工具RESPONSE)都呼叫這個 → 全部對話都會進記錄。"""
        try:
            if getattr(self, "_hm_logged", False):
                return
            _u = (getattr(self, "_hm_user", "") or "").strip()
            _r = (reply or "").strip()
            if not (_u and _r):
                return
            self._hm_logged = True
            import urllib.request as _cu, json as _cj, threading as _th2
            def _send(uu, rr):
                try:
                    _cu.urlopen(_cu.Request(
                        "http://host.docker.internal:8811/api/chat/log",
                        data=_cj.dumps({"user": uu, "assistant": rr}).encode(),
                        headers={"Content-Type": "application/json"}), timeout=3)
                except Exception:
                    pass
            _th2.Thread(target=_send, args=(_u, _r), daemon=True).start()
        except Exception:
            pass

    def _trim_history(self, keep_turns=6):
        """對話歷史滑動視窗：只留最近 keep_turns 個使用者回合。每輪都把全部歷史(含表情工具呼叫+
        thought_signature 大簽章)堆進上下文 → 越後面的回合越慢；截斷掉舊的就讓延遲穩定。
        從 user 訊息邊界切，不會切斷 tool_call/tool_result 配對(否則上游會 400)。"""
        try:
            msgs = self.dialogue.dialogue
            user_idxs = [i for i, m in enumerate(msgs)
                         if getattr(m, "role", "") == "user"
                         and not getattr(m, "is_temporary", False)]
            if len(user_idxs) <= keep_turns:
                return
            cut = user_idxs[-keep_turns]   # 從倒數第 keep_turns 個 user 開始保留
            self.dialogue.dialogue = [
                m for i, m in enumerate(msgs)
                if getattr(m, "role", "") == "system"
                or getattr(m, "is_temporary", False)
                or i >= cut
            ]
        except Exception:
            pass
    ...
    def _speak_text(self, text):
        """讓裝置【主動】講一句話(注入 TTS 流程)——給提醒/通知到點時開口用。"""
        try:
            sid = uuid.uuid4().hex
            self.sentence_id = sid
            self.tts.tts_text_queue.put(TTSMessageDTO(
                sentence_id=sid, sentence_type=SentenceType.FIRST, content_type=ContentType.ACTION))
            self.tts.tts_text_queue.put(TTSMessageDTO(
                sentence_id=sid, sentence_type=SentenceType.MIDDLE,
                content_type=ContentType.TEXT, content_detail=text))
            self.tts.tts_text_queue.put(TTSMessageDTO(
                sentence_id=sid, sentence_type=SentenceType.LAST, content_type=ContentType.ACTION))
            self.logger.bind(tag=TAG).info(f"主動語音播報: {str(text)[:30]}")
        except Exception as e:
            self.logger.bind(tag=TAG).warning(f"_speak_text 失敗: {e}")

    async def _voice_reminder_poller(self):
        """每 15 秒去 8809 撈待播語音(提醒到點 daemon 會丟進佇列)→ 讓 StackChan 主動開口叫使用者。
        只講 3 分鐘內的(避免裝置離線後重連時把舊提醒亂報出來)。"""
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if self.stop_event.is_set():
                    break
                # 裝置正在講話 / 對話進行中 → 這輪先別撈待播。/pending_voice 會清空佇列，
                # 若此時插入提醒會搶走 sentence_id、把正在講的回答切掉。跳過→佇列留在 8809，
                # 等它講完下一輪(≤15s)再撈出來講，提醒最多晚一點、但絕不打斷回答。
                if (getattr(self, "client_is_speaking", False)
                        or getattr(self, "_hermes_in_turn", False)
                        or getattr(self, "_hermes_in_continuous", False)):
                    continue

                def _fetch():
                    try:
                        r = _u.urlopen(
                            "http://host.docker.internal:8809/pending_voice", timeout=4)
                        return _j.loads(r.read())
                    except Exception:
                        return None
                data = await self.loop.run_in_executor(None, _fetch)
                ...
                for it in data["items"]:
                    if isinstance(it, dict):
                        ...
                        txt, ts = str(it).strip(), now
                    if txt and (now - ts) < 180:
                        self._speak_text(txt)
                        await asyncio.sleep(7)   # 多則之間隔開,別黏在一起
            except Exception:
                pass

    def _hermes_is_noise(self, query):
        """判斷 ASR 結果是不是噪音/亂碼/幻覺(持續對話免喚醒時,背景聲會被聽成這些)。
        只擋【明確的垃圾】(空/純標點/高度重複/已知幻覺字串)，不誤殺正常短句('好''幾點')。"""
        try:
            import json as _j2, zlib as _z2
            t = str(query or "").strip()
            if t.startswith("{"):
                try:
                    t = (_j2.loads(t).get("content") or t).strip()
                except Exception:
                    pass
            if not t:
                return True
            ...
            if all((c in "。，、．.,!?！？；;：:~～…—-_ \t\n\"'`") for c in t):
                return True
            # 高度重複(壓縮率超低)= ASR 卡住狂吐同字:「想要想要…」「100原本100原本…」
            b = t.encode("utf-8")
            if len(b) >= 14:
                try:
                    if (len(_z2.compress(b, 6)) / len(b)) < 0.40:
                        return True
                except Exception:
                    pass
            # 字元重複率:8字以上但「不同字/總字」太低 = 重複噪音(「全域性全域性全域性…」)
            _ct = [c for c in t if not c.isspace()]
            if len(_ct) >= 8 and (len(set(_ct)) / len(_ct)) < 0.32:
                return True
            # 已知幻覺/字幕雜訊字串(Whisper/FunASR 對非語音常吐這些)
            for h in ("謝謝觀看", "谢谢观看", "請不吝", "请不吝", "訂閱", "订阅", "點贊", "点赞",
                      "字幕", "promoting", "初音", "ミク", "MBC", "下次再見"):
                if h in t:
                    return True
            return False
        except Exception:
    ...
    def _hermes_should_filler(self, query):
        """判斷這句是不是『需要查/算、會花幾秒』的問題 → 值得先講一句墊場讓體感秒回。
        打招呼、短閒聊、命令式動作(放歌/關燈)不墊(本來就快,墊了反而怪)。"""
        try:
            q = str(query or "")
            if q.strip().startswith("{"):
                import json as _fj
                q = str(_fj.loads(q).get("content") or q)
            _kw = ("查", "多少", "幾", "股票", "投資", "賺", "虧", "報酬", "淨值", "資產",
                   "花", "費", "消費", "天氣", "氣溫", "下雨", "附近", "找", "搜", "哪",
                   "誰", "什麼時候", "幾點", "幾號", "算", "統計", "新聞", "價", "匯率",
                   "怎麼", "為什麼", "翻譯", "提醒")
            return any(k in q for k in _kw)
        except Exception:
            return False
    ...
    def chat(self, query, depth=0):
        ...
        if query is not None:
            ...
            # 【C 噪音/亂碼過濾】持續對話免喚醒時,背景雜音常被 ASR 聽成亂碼/重複/幻覺。
            # 判定是噪音就【安靜略過】：不回應、也不碰持續視窗(讓它自然超時回待命,不被雜音一直延長)。
            if depth == 0 and self._hermes_is_noise(query):
                self.logger.bind(tag=TAG).info(f"🔇 判定噪音/亂碼,安靜略過: {str(query)[:36]}")
                return True
            # 【A 聲紋過濾】僅在「持續對話免喚醒視窗內」生效:若這段聲音不是主人(背景人聲/電視被聽成話)
            # → 安靜略過。只有聲紋有啟用(voiceprint_provider)且明確判定「未知说话人」才擋,避免誤殺主人。
            # 喚醒詞那一輪(非持續視窗)不過濾——使用者剛喚醒就是在跟它講話。
            # 【A 聲紋過濾】暫時停用(if False)：目前聲紋認不準、會把主人誤判成「未知」，
            # 這個過濾就會把主人講話靜音丟掉(沒回應/沒逐字稿)。等聲紋重新註冊、認得準再開。
            # 背景噪音仍由內容噪音過濾(B)擋。
            if False and (depth == 0
                    and getattr(self, "_hermes_in_continuous", False)
                    and getattr(self, "voiceprint_provider", None) is not None
                    and getattr(self, "current_speaker", None) == "未知说话人"):
                self.logger.bind(tag=TAG).info("🔇 持續對話中、非主人聲音,安靜略過")
                return True
            # Hermes 持續對話：新對話進來 → 標記處理中 + bump epoch 讓還在睡的待命 watcher 失效，
            # 並取消它，避免處理中被誤判沉默而把裝置踢回待命。
            if depth == 0:
                self._hermes_in_turn = True
                self._hermes_listen_epoch = getattr(self, "_hermes_listen_epoch", 0) + 1
                _t = getattr(self, "_hermes_standby_task", None)
                if _t is not None and not _t.done():
                    _t.cancel()

        # 为最顶层时新建会话ID和发送FIRST请求
        if depth == 0:
            # 把聲紋結果【原樣】送給 8809，由 sync_identity 判斷(認出本人/訪客→確認;
            # 未知→sticky維持上次確認 or 設unknown讓大腦問是誰)。聲紋沒作用(空)→當主人。
            try:
                # 【提速】identity/set 的回應從沒被讀(伺服器端 sticky)→ 移到背景執行緒，
                # 省掉關鍵路徑最多 2s 阻塞。橋接的 identity/turn 在一整個 LLM 請求後才讀，趕得上。
                import urllib.request as _ur, json as _uj, threading as _idth
                _spk = (getattr(self, "current_speaker", None) or "").strip()

                def _id_set(_s=_spk):
                    try:
                        _ur.urlopen(_ur.Request("http://host.docker.internal:8809/identity/set",
                            _uj.dumps({"speaker_id": _s if _s else "owner", "name": ""}).encode(),
                            {"Content-Type": "application/json"}), timeout=2)
                    except Exception:
                        pass
                _idth.Thread(target=_id_set, daemon=True).start()
            except Exception:
                pass
            current_sentence_id = str(uuid.uuid4().hex)
            self.sentence_id = current_sentence_id  # 更新共享属性
            self.dialogue.put(Message(role="user", content=query))
            # Hermes：記下這一輪的原始問句 + 重置「已記錄」旗標。回覆若來自工具(在 depth+1 生成)
            # 也要能記進 dashboard ——所以記錄改在「產生回覆的那個 depth」做、帶這個原始問句。
            try:
                import json as _cj0
                _hu = str(query).strip()
                if _hu.startswith("{"):
                    _hu = (_cj0.loads(_hu).get("content") or _hu).strip()
                self._hm_user = _hu
            except Exception:
                self._hm_user = str(query).strip()
            self._hm_logged = False
            # Hermes：對話時轉頭面向使用者（lifelike look-at-user）
            self._look_at_user(query)
            ...
            # Hermes 秒回墊場：查詢類問題要花幾秒查工具,先立刻講一句自然的墊場(像真人「我看看喔」)。
            # 跟正式回覆同一個 sentence_id → 接成同一句話,使用者體感「秒回」而不是乾等。
            # 打招呼/閒聊(_should_filler=False)不墊,免得每句都「我看看」很怪。
            try:
                if self._hermes_should_filler(self._hm_user):
                    import random as _frnd
                    _filler = _frnd.choice(
                        ["嗯，我看看喔。", "好，我查一下。", "讓我看看。", "嗯，我確認一下。"])
                    self.tts.tts_text_queue.put(
                        TTSMessageDTO(
                            sentence_id=current_sentence_id,
                            sentence_type=SentenceType.MIDDLE,
                            content_type=ContentType.TEXT,
                            content_detail=_filler,
                        )
                    )
            except Exception:
                pass
        ...
        if (
            ...
            if DIRECT_ANSWER_ENABLED and functions is not None and depth == 0:
        ...
        try:
            # 使用带记忆的对话
            memory_str = None
            # 【提速】self_state 先在背景抓(跟記憶查詢並行)，原本兩個串行最壞 8s+2s → 現在重疊。
            _ss_holder = {}
            if query:
                try:
                    import threading as _ssth, urllib.request as _ssu, urllib.parse as _ssup, json as _ssjj

                    def _ss_fetch():
                        try:
                            _rr = _ssu.urlopen(
                                "http://host.docker.internal:8809/self_state?channel=" + _ssup.quote("語音"),
                                timeout=2)
                            _ss_holder["text"] = _ssjj.loads(_rr.read().decode()).get("text", "")
                        except Exception:
                            pass
                    _ss_t = _ssth.Thread(target=_ss_fetch, daemon=True)
                    _ss_t.start()
                    _ss_holder["thread"] = _ss_t
                except Exception:
                    pass
            ...
            if self.memory is not None and query:
                ...
                try:
                    memory_str = future.result(timeout=8)  # 8809 卡住也不會凍住整輪
                except Exception:
                    memory_str = None

            # Hermes 自我認知：每輪注入「即時真實狀態」（能力/服務/時間/位置，全部從實際系統生成）。
            if query:
                # 裝置心跳 + 即時推播「使用者開口了」給 dashboard（純 fire-and-forget）
                try:
                    import threading as _th, urllib.request as _su0, json as _sj0
                    _qtext = str(query)
                    if _qtext.startswith("{"):
                        try:
                            _qtext = _sj0.loads(_qtext).get("content") or _qtext
                        except Exception:
                            pass
                    def _ping():
                        try:
                            _su0.urlopen(_su0.Request(
                                "http://host.docker.internal:8809/device_ping", data=b"{}",
                                headers={"Content-Type": "application/json"}), timeout=2)
                        except Exception:
                            pass
                        try:
                            _su0.urlopen(_su0.Request(
                                "http://host.docker.internal:8811/api/event",
                                data=_sj0.dumps({"type": "engaged", "data": _qtext[:60]}).encode(),
                                headers={"Content-Type": "application/json"}), timeout=2)
                        except Exception:
                            pass
                    ...
                    pass
                # self_state 要併進 prompt——已在上面跟記憶查詢【並行】預抓，這裡只收結果(最多再等2s)。
                try:
                    _ss_t = _ss_holder.get("thread")
                    if _ss_t is not None:
                        _ss_t.join(timeout=2)
                    _ssb = _ss_holder.get("text", "")
                    if _ssb:
            ...
            # Hermes 修：LLM 一開始就失敗時，仍要送一段話 + LAST，否則裝置會卡在 speaking、
            # 也不會觸發持續對話。給使用者一句友善的錯誤回應。
            if depth == 0:
                try:
                    self.tts.tts_text_queue.put(TTSMessageDTO(
                        sentence_id=current_sentence_id, sentence_type=SentenceType.MIDDLE,
                        content_type=ContentType.TEXT,
                        content_detail="我剛剛恍神了一下，可以再說一次嗎？"))
                except Exception:
                    pass
                try:
                    self.tts.tts_text_queue.put(TTSMessageDTO(
                        sentence_id=current_sentence_id, sentence_type=SentenceType.LAST,
                        content_type=ContentType.ACTION))
                except Exception:
                    pass
            return None
        ...
        if len(response_message) > 0:
            ...
            # Hermes：純文字 / 工具 REQLLM 回覆走這條 → 記進 dashboard
            self._hermes_log_chat(text_buff)
        ...
        if depth == 0:
            ...
            # 這一輪結束 → 修剪對話歷史(滑動視窗),讓後面的回合不會越來越慢
            self._trim_history(keep_turns=6)
    ...
    def _handle_function_result(self, tool_results, depth, streamed_text=""):
        ...
        for result, tool_call_data in tool_results:
            if result.action in [
                ...
                self._hermes_log_chat(text)   # 工具 RESPONSE/錯誤回覆(天氣/開工/記帳確認)也記進 dashboard
        ...
        if record_tools:
            ...
            if response_parts:
                ...
                self._hermes_log_chat("，".join(response_parts))   # RECORD 型工具回覆也記
