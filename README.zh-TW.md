...
## 💼 工作雷達 — 上傳履歷,AI 每天幫你挑對的工作

![工作雷達](docs/screenshots/jobs.png)

不是關鍵字比對,是**讀懂你的履歷之後,拿每個職缺跟「你」逐一評分**。

- **上傳履歷(PDF)** → AI 讀懂你的背景、專長、目標,生成配對輪廓與搜尋關鍵字。換人用只要換一份履歷。
- **每天自動掃**(12:35)Yourator + LinkedIn,LLM 按你的履歷逐缺評**適配分 0–100 + 一句理由**,新出現的高分職缺推到 Telegram。
- **英文職缺自動附中文翻譯**(標題 + 完整 JD),點「看中文說明」即時抓原文翻譯。
- **自訂求職條件** — 用白話寫「遠端優先、月薪 7 萬以上、想做 AI 導入顧問、不要金融業」,AI 整理成配對標準,之後掃描以此為最高優先。
- **不喜歡按 ✕** — 該職缺不再出現,還變成 AI 的口味負樣本,越推越準。
- **☆ 關注** → 存進追蹤清單反覆看;**🔬 深度分析** → 真的上網研究公司風評、薪資行情、JD 跟你履歷的差距與補強、面試準備,結果快取可反覆看。
- **📨 LinkedIn 一鍵投遞** — 開一個看得見的視窗自動 Easy Apply + 上傳履歷,遇到沒把握的欄位停下來把視窗留給你手動完成(寧可少按一步,不亂送)。
- **口語操作** — 在 Telegram/語音直接說「有什麼新工作」「Google 那個缺的詳情」「幫我追蹤」「深度研究它」,同一套工具全通。

> 通用版(任何人上傳履歷找工作)獨立開源在 [owen4sure/job-radar](https://github.com/owen4sure/job-radar)。
